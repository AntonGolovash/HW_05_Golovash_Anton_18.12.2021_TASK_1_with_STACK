#include "Stack.h"
template<typename T>
Stack<T>::Stack() : _data(nullptr), _size(0), _capacity(20)
{
...
Stack<T>::Stack(Stack&& other) : _data(nullptr), _size(0)
{
	...
	_data = other._data;
	_size = other._size;
	other._data = nullptr; // ѕрисвоить указателю nullpter что бы деструктор лишний раз не срабатывал
	other._size = 0;
...
template<typename T>
ostream& operator<< (ostream& out, const Stack<T>& stack) // «десь стандартна€ реализаци€
{
